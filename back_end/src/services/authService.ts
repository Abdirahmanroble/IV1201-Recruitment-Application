...
class AuthService {
  ...
  public static async register ({ name, surname, pnr, email, username, password }: registerCredentials): Promise<User | string> {
    try {
      ...
      const user = await User.create({ name, surname, pnr, email, username, password: hash })
